...
Clones all sibling repos (GhostProxy, RootFS, ToolCore, FlowSpec, JudgementD) and builds GhostProxy, RootFS, control-plane. No temp files, no credential prompts.

**Next steps** (after setup):

```bash
# 1. Add your Anthropic key to .env (or export SECRET_ANTHROPIC_KEY)
echo "anthropic_key=sk-ant-your-key" >> .env

# 2. Bootstrap config (one-time)
./build/control-plane onboard --secrets-provider env

# 3. Run the hello-world agent (starts proxy automatically)
cd examples/hello-world && ../build/control-plane run
```
...
Put secrets in a `.env` file (e.g. `anthropic_key=sk-ant-...`) or export `SECRET_ANTHROPIC_KEY`, etc. See [docs/secrets-local-dev.md](docs/secrets-local-dev.md).
...
../build/control-plane run
```

That's it. `run` restarts/starts GhostProxy, boots the sandbox, and tails logs.
